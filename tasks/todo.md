...
## المرحلة VI — اختبارات E2E بـPlaywright (خطة `shimmering-swimming-hummingbird.md`)

مرجع: `C:\Users\HP\.claude\plans\shimmering-swimming-hummingbird.md`. أكبر فجوة جودة موثَّقة (صفر اختبارات متصفح حي عبر كل المشروع). قرارات المستخدم المسبقة: استخدام `human-owner@bab.local` (يحتاج كلمة المرور)، إنشاء طاولة اختبار جديدة عبر سكربت (بدل استخدام بيانات حية)، واختبار الولاء جزئياً بجلسة Supabase محقونة (بلا Google OAuth حي).

- [x] `npm i -D @playwright/test` + `npx playwright install chromium`
- [x] `playwright.config.ts` — `baseURL` قابل للتخصيص عبر `PLAYWRIGHT_BASE_URL`، `webServer` يُشغّل `npm run dev` تلقائياً (أو يعيد استخدام خادم قائم فعلاً)، `locale: "ar-SA"` (لازم فعلياً — بدونه Chromium يرسل `Accept-Language: en` فتُعرض الواجهة إنجليزية وتفشل كل الاختبارات التي تبحث عن نصوص عربية)
- [x] `scripts/e2e/seed.ts` — ينشئ (أو يعيد استخدام) طاولة/QR اختبار تحت أول فرع فعلي يملك أصنافاً حقيقية (اختير ديناميكياً، وليس مطعم "باب البلد" بالاسم لأن قاعدة البيانات الحية تضم عدة مطاعم اختبارية بلا أصناف)
- [x] `scripts/e2e/reset-test-user-password.ts` — يضبط كلمة مرور `human-owner@bab.local` عبر Admin API (الحساب موجود مسبقاً من المرحلة 5، بلا كلمة مرور معروفة)
- [x] `scripts/e2e/create-loyalty-session.ts` — ينشئ مستخدم اختبار + عضوية ولاء، ويسجّل دخوله عبر supabase-js مباشرة ملتقطاً الكوكيز التي تكتبها `@supabase/ssr` نفسها (بدل إعادة تنفيذ منطق ترميز/تقطيع الكوكي يدوياً) لحقنها لاحقاً بـ`context.addCookies`
- [x] `tests/e2e/reservation.spec.ts` — حجز عام من `/site/reserve` حتى ظهور بطاقة التأكيد
- [x] `tests/e2e/qr-menu.spec.ts` — فتح رابط QR وتصفح المنيو وفتح تفاصيل صنف
- [x] `tests/e2e/login-reservation.spec.ts` — دخول `human-owner@bab.local` + إنشاء حجز من لوحة الإدارة حتى صفحة التفاصيل
- [x] `tests/e2e/loyalty-partial.spec.ts` — جزئي كما تقرَّر: جلسة محقونة بدل Google OAuth حي، ورفع صورة اختبار بلا افتراض نتيجة OCR محدَّدة (نجاح أو رفض، المهم عدم التعليق/العطل)
- [x] `npm run test:e2e` في `package.json`
- [x] `vitest.config.mts` جديد (لم يكن موجوداً) لاستثناء `tests/e2e/**` من `npm test` — بدونه vitest يحاول تشغيل ملفات Playwright كاختبارات عادية ويفشل
- [x] قسم جديد في `README.md` عن التشغيل والمتطلبات وسبب كون اختبار الولاء جزئياً
- [x] `npm run build` + `npm run lint` + `npm test` + `npx tsc --noEmit` كلها نظيفة، و`npx playwright test` (4/4) تمر فعلياً ضد خادم `next dev` حي

### مراجعة (Review) — المرحلة VI ✅ (منجزة 2026-09-17)

بناء الاختبارات الأربعة كشف **ثغرتَي إنتاج حقيقيتين موجودتين مسبقاً**، بالضبط كما تنبّأ نص الخطة ("أكبر فجوة جودة") — كلتاهما بلا علاقة مباشرة ببنية الاختبارات نفسها، واكتُشفتا فقط لأن الاختبارات تُشغَّل من متصفح حي بجلسات حقيقية بدل محاكاة/service_role كما في كل التحقّقات السابقة:

1. **كل صفحات `/site/*` (الموقع التسويقي العام، المرحلة 16) كانت تُحوَّل تلقائياً إلى `/login`** لأي زائر مجهول. السبب: مصفوفة `matcher` في `proxy.ts` استثنت `/m/*` و`/auth/*` فقط من فحص المصادقة، ونسيت `/site/*` بالكامل رغم كونه تحت مجموعة `(public)` صراحةً. **كل زائر حقيقي غير مسجَّل دخول لصفحة الحجز العامة/المنيو التسويقي/التواصل كان يُحوَّل لصفحة الدخول فوراً منذ إطلاق المرحلة 16** — على الأرجح غير ملحوظ لأن كل اختبار سابق واختبار المتصفح اليدوي جرى من متصفح المطوّر وهو مسجَّل دخوله كمدير غالباً. **الإصلاح**: تعديل الـregex في `proxy.ts` ليستثني `site(?:/|$)` أيضاً (سطر واحد).
2. **صفحة الولاء العامة (`/m/[branchSlug]/loyalty`) كانت تُرجع 404 دائماً لأي عميل ولاء حقيقي سجّل دخوله فعلياً عبر Google** (المرحلة 19). السبب أعمق: `resolveBranch()`/`resolveTable()` يقرآن مباشرة من `branches`/`restaurants`/`tables`/`table_qr_codes` عبر عميل RLS-scoped عادي. سياسات القراءة العامة على هذه الجداول (منذ المرحلتين 5/6) مكتوبة `for select to anon` حصراً — بمجرد أن يسجّل الزائر دخوله فعلياً (دوره يتحوّل من `anon` إلى `authenticated`)، تتوقف سياسة anon عن الانطباق ويُطبَّق بدلاً منها `branches_select`/`restaurants_select` الموظَّفي (يتطلب صف `profiles`)، فتُرجع الاستعلامات صفراً من الصفوف دائماً لأي عميل ولاء (ليس موظفاً). **محاولة إصلاح أولى فاشلة**: أضفتُ هجرة `0026` بسياسة `select` إضافية "لـauthenticated بلا شرط" — كشف اختبار الحجز (تسجيل الدخول) الفشل فوراً: سياسات RLS لنفس العملية والدور تُدمَج بـ`OR`، فسياسة "authenticated بلا شرط مطعم" أبطلت عملياً عزل الفروع بين المطاعم لكل الموظفين أيضاً (موظف مطعم صار يرى فروع مطعم آخر بالكامل في كل شاشات الإدارة). **الإصلاح الصحيح** (هجرة `0027`، بعد التراجع عن `0026`): دالتا `security definer` جديدتان (`resolve_branch_public`, `resolve_table_public`) بنفس نمط `get_table_reservation` الموجود مسبقاً منذ المرحلة 8 — تتجاوزان RLS داخلياً لإرجاع الأعمدة العامة فقط، بلا أي تعديل على سياسات RLS نفسها. `resolveBranch()`/`resolveTable()` تستدعيان الدالتين بدل القراءة المباشرة.
- **أثر جانبي إيجابي غير مقصود**: إصلاح #2 يُصحّح أيضاً مسارات `/m/[branchSlug]/t/[qrToken]` (الرئيسية)، `/complaint`، `/rating` — كلها تستخدم نفس `resolveTable()`، وكانت جميعها ستفشل بنفس الطريقة لأي زائر مسجّل دخول (مثلاً عميل ولاء يعود لمسح QR بعد تسجيل الدخول).
- **قيد معروف غير مُصلَح (خارج نطاق هذه المرحلة)**: `npm run db:migrate` يفشل حالياً عند `0024_loyalty_staff_redeem.sql` بخطأ "function already exists" — دالة تلك الهجرة مُطبَّقة فعلاً على القاعدة الحية لكن الهجرة غير مسجَّلة في جدول `_migrations` (على الأرجح تطبيق يدوي سابق من جلسة أخرى). لم ألمس هذا لأنه خارج نطاق مهمة الاختبارات ويحتاج قراراً من المستخدم (تسجيلها كمُطبَّقة يدوياً أم إعادة فحص محتواها أولاً)؛ طبّقتُ `0026`/`0027` مباشرة بسكربتات مؤقتة منفصلة (حُذفت بعد الاستخدام) بدل `scripts/migrate.ts` لتفادي هذا العطب.
- **اختبار الولاء يبقى جزئياً بالتصميم** (قرار مستخدم مسبق): لا يقود شاشة Google الحقيقية، ولا يفترض نتيجة استخراج OCR محدَّدة لأن صورة الاختبار مصنَّعة وليست فاتورة حقيقية (استدعاء OpenRouter حقيقي بمفتاح حقيقي — كل تشغيل للاختبار يستهلك طلب API فعلي).
- **إصلاح إضافي صغير غير متعلّق بـRLS**: زر/حقل "وقت الحجز" (`Select` نصف الوقت) في `ReserveTableForm.tsx` (الحجز العام) كان بلا `FormControl` محيطة، فلم يكن للعنصر `id` يطابق `htmlFor` في `FormLabel` — عطل وصولية (a11y) حقيقي (قارئ الشاشة لا يربط التسمية بالحقل) لم يظهر إلا لأن اختبار Playwright استخدم `getByLabel` فعلياً كما يفعل مستخدم تقنية مساعدة.
- **بيانات الاختبار المُنشأة على القاعدة الحية ولم تُحذف عمداً** (تُعاد استخدامها في كل تشغيل لاحق، idempotent): طاولة/QR اختبار على الفرع الفعلي الوحيد بأصناف حقيقية، ومستخدم + عضوية ولاء اختباريان (`e2e-loyalty@bab.local`).
